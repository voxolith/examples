...
| [`valley`](src/valley/main.ts) | the world's valley at 100 voxels per metre from the same layout (`world/layout.ts`): generators refined to 1 cm voxels (`{ voxelsPerMetre: 100 }`), every model drawn as instances (`makeInstanceLayer`, `orientationYaw`), `refineTerrain` ground with grass blades streamed in chunks, real-size rats as an instanced crowd; `?variants=`, `?rats=`, `?radius=` | ~390 |

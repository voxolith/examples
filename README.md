...
| [`valley`](src/valley/main.ts) | the world's valley at 100 voxels per metre from the same layout (`world/layout.ts`): generators refined to 1 cm voxels (`{ voxelsPerMetre: 100 }`), every model drawn as instances (`makeInstanceLayer`, `orientationYaw`), `refineTerrain` ground with grass blades streamed in chunks, real-size rats as an instanced crowd; models cached in IndexedDB after the first visit; phones get 50 vox/m and a smaller valley (`?full` for all); `?vpm=`, `?variants=`, `?rats=`, `?radius=` | ~390 |
| [`instances`](src/instances/main.ts) | six tree models placed hundreds of times at any heading and a crowd of rats drawn by reference (`addModel`, `setInstances`, `makeCrowd({ instances })`); `?stamp` to compare with stamping | ~150 |
